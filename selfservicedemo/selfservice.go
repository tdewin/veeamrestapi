...
func (h *SelfService) setTemplates(logoimg string,hoster string) (error) {
	var err error
...
<html>
	...
	<body>
		<div class="header">
			<!--logostart--><span style="font-size:40px;">Availability hosting</span><!--logoend-->
			<br>Self Service Demo
		</div>
		<div class="content">
	...
	//<!--logostart--><span style="font-size:40px;">Availability hosting</span><!--logoend-->
	if logoimg != "" {
		tpl = strings.Replace(tpl,`<!--logostart--><span style="font-size:40px;">Availability hosting</span><!--logoend-->`, "<img src='/___staticfileslogo.png'/>", 1)
		log.Printf("Replaced header by logo")
	} else if hoster != "" {
		tpl = strings.Replace(tpl,`<!--logostart--><span style="font-size:40px;">Availability hosting</span><!--logoend-->`, "<!--logostart--><span style=\"font-size:40px;\">"+hoster+"</span><!--logoend-->", 1)
		log.Printf("Replaced header by name")
	}
...
func (h *SelfService) cacheStatic(logofile string) (error) {
	var err error
	...
	if logofile != "" {
		var b []byte
		b, err = ioutil.ReadFile(logofile)
		if err == nil {
			files["logo.png"] = b
		} 
	}
...
func main() {
	...
	flogoimg := flag.String("logo","","local logo in the same dir, e.g logo.png (has to be png)")
	fhoster := flag.String("hoster","","Hoster name")
	flag.Parse()
	...
	if err == nil {
		...
		err = selfservice.cacheStatic(logoimg)
		if err != nil {
			log.Printf("Could not read logo, reseting")
			logoimg = ""
		}
		err = selfservice.setTemplates(logoimg,*fhoster)
		if err == nil {
